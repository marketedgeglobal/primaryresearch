...
Usage:
    python scripts/validate_schema.py --schema schemas/analysis_schema.json --input analyses/analysis-run-1234.json --output-dir analyses
...
from output_writer import write_error_output
...
def main() -> None:
    ...
    try:
        cfg = load_config({"output_dir": args.output_dir})
    ...
    try:
        ...
        error_path = write_error_output(run_id, error_payload, cfg["output_dir"])
        log(f"[{run_id}] Wrote error output: {error_path}")
